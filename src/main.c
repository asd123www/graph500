...
int main(int argc, char** argv) {
	// argc = 3, argv[0] = 0, argv[1] = SCALE, argv[2] = edgefactor.
	aml_init(&argc,&argv); //includes MPI_Init inside
	setup_globals();

	/* Parse arguments. */
	// SCALE规定图的大小, 节点个数为 (1<<SCALE)
	// edgefactor 表示平均每个点的degree.
	int SCALE = 16;
	 ...
	 * validator). */
	int num_bfs_roots = 1; // 验证次数, 次数更多则统计误差更小.
	...
	// 从这里可以看出上面的shit block就是在准备graph.
	...
	make_graph_data_structure(&tg); // 我们自己写的函数.
	...
	// 生成每个测试用例的root, 由于seed相同因此不同process数据相同.
	{
		uint64_t counter = 0;
		int bfs_root_idx;
	...
	if (!getenv("SKIP_BFS")) {
		...
		run_bfs(bfs_roots[0], &pred[0]); //warm-up, 热身,意义是啥??? 会不会是观察到这样可以减轻第一次bfs由cache带来的slow-down
		...
		for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
			...
			if (rank == 0) fprintf(stderr, "asd123www: Running BFS %d\n", bfs_root_idx);

			clean_pred(&pred[0]); //user-provided function from bfs_implementation.c
			/* Do the actual BFS. */
			double bfs_start = MPI_Wtime();
			run_bfs(root, &pred[0]); // 运行bfs算法
			double bfs_stop = MPI_Wtime();
			bfs_times[bfs_root_idx] = bfs_stop - bfs_start; // 统计时间.
